...
func (j *JWTService) ValidateToken(tokenString string) (*JWTClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &JWTClaims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, customErr.NewError(customErr.INTERNAL_ERROR, "Unexpected signing method", http.StatusInternalServerError, nil)
		}
	...
	return nil, customErr.NewError(customErr.INTERNAL_ERROR, "Invalid token", http.StatusInternalServerError, nil)
